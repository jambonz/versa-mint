...
const onCodeGather = async (session, evt) => {
  ...
  if (reason !== 'dtmfDetected' || !digits) {
    logger.info({reason}, 'no code received, hanging up');
    session
      .hangup({
        headers: {
  ...
  try {
    ...
    if (!response.ok || !body.success || !body.phone_number) {
      logger.info({status: response.status, body}, 'code lookup failed, hanging up');
      session
        .hangup({
          headers: {
    ...
    session.locals.currentCallerId = body.phone_number;
    session
      .dial({
        answerOnBridge: true,
        anchorMedia: true,
